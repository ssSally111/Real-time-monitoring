...
def pipeline_coreml(model, im, file, names, y, prefix=colorstr('CoreML Pipeline:')):
    ...
    if platform.system() == 'Darwin':
        img = Image.new('RGB', (w, h))  # train(192 width, 320 height)
        # train = torch.zeros((*opt.img_size, 3)).numpy()  # train size(320,192,3) iDetection
...
def parse_opt(known=False):
    ...
    parser.add_argument('--imgsz', '--train', '--train-size', nargs='+', type=int, default=[640, 640], help='image (h, w)')
